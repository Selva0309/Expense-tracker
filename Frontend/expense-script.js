...
function addexpense() {
    ...
    axios.post('http://18.181.82.118/expenses/addexpense',{description: description, category:category, type: type, amount:amount},{headers: {"Authorization": token}})
    .then((response)=>{
...
function showexpense(page){
    const token = localStorage.getItem('token');
    axios.get('http://18.181.82.118/expenses/expenselist', {headers: {"Authorization": token,'page':page, 'limit': limit }})
    .then((response)=>{
...
function deleteexpense(id){
    const token = localStorage.getItem('token');
    axios.post('http://18.181.82.118/expenses/delete-expense',{expenseId: id}, {headers: {"Authorization": token}})
    .then((response)=>{
        message = response.data.message;
        notifyUser(message);
        showexpense();
...
async function razorpay() {
    const token = localStorage.getItem('token');
    axios.get('http://18.181.82.118/purchase/premiummembership', {headers: {"Authorization": token}})
    .then((response)=>{
        console.log(response);
    var options = {
            ...
            "handler" : async function(response) {
                axios.post('http://18.181.82.118/purchase/updatetransactionstatus', {
...
function getdashboard(){
    axios.get('http://18.181.82.118/premium/dashboard')
    .then(response=>{
        console.log(response);
    })
}
